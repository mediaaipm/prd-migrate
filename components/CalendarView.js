    ...
    <div className="cal-view">
      ...
      {openDay && (
        <div
          ...
          onClick={e => { if (e.target === e.currentTarget) { setOpenDay(null); setEditId(null) } }}
        >
          <div style={{ background: '#fff', borderRadius: 14, width: '100%', maxWidth: 520, boxShadow: '0 20px 60px rgba(0,0,0,.18)', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
            <div style={{ padding: '18px 22px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button onClick={() => { setOpenDay(null); setEditId(null) }} style={{ background: 'none', border: 'none', fontSize: 20, cursor: 'pointer', color: '#94a3b8', lineHeight: 1 }}>×</button>
            </div>
            <div style={{ padding: '16px 22px', overflowY: 'auto' }}>
              {editId && editId !== 'new' ? (
                ...
                <>
                  ...
                  <button className="btn-primary" style={{ fontSize: 13, padding: '7px 16px' }} onClick={() => setEditId('new')}>+ Add task on this day</button>
                </>
              )}
            </div>
          </div>
