...
function buildChanges(before, updates) {
  ...
  for (const field of FIELD_ORDER) {
    ...
    if (field === 'checklist') {
      const norm = list => (Array.isArray(list) ? list : []).map(i => `${i.id}:${i.done ? 1 : 0}:${i.text}`)
      const from = norm(before.checklist)
      const to = norm(updates.checklist)
      if (JSON.stringify(from) !== JSON.stringify(to)) {
        const count = list => `${(Array.isArray(list) ? list : []).filter(i => i.done).length}/${(Array.isArray(list) ? list : []).length} done`
        changes.push({ field, label: FIELD_LABEL[field], from: count(before.checklist), to: count(updates.checklist) })
      }
      continue
    }
    if (field === 'cover') {
