...
// Starting number for a project's task ids (e.g. start at ENG-1001). Positive
// integer; anything invalid falls back to 1.
function normalizeTaskSeqStart(v) {
  const n = Number(v);
...
async function createProject(name, description, { status, priority, members, taskPrefix, taskSeqStart } = {}) {
  ...
  const meta = {
    ...
    taskSeqStart: normalizeTaskSeqStart(taskSeqStart),
...
async function updateProject(slug, { name, description, status, priority, members, taskAcl, taskPrefix, taskSeqStart }) {
  const meta = await getKv().get(`project:${slug}`);
  if (!meta) return null;
  const updated = {
    ...meta,
    ...
    ...(taskSeqStart !== undefined ? { taskSeqStart: normalizeTaskSeqStart(taskSeqStart) } : {}),
...
async function deleteProject(slug) {
  ...
  await Promise.all([
    ...
    getKv().del(`columns:${slug}`),
    getKv().del(`categories:${slug}`),
...
async function deleteVersion(slug, version) {
  const removed = await getKv().srem(`versions:${slug}`, version);
  if (!removed) return false;
  // Attachment bytes sit in their own keys, so dropping the task list would strand
  // them. Read the list before deleting it.
  const { deleteTasksMedia } = require('./task-media');
  const doomed = await getKv().get(`tasks:${slug}:${version}`);
  await Promise.all([
    getKv().del(`version:${slug}:${version}`),
    getKv().del(`tasks:${slug}:${version}`), // version-scoped task list
  ]);
  await deleteTasksMedia(slug, version, Array.isArray(doomed) ? doomed : []);
  await refreshProjectStats(slug); // recomputes latestVersion from remaining versions
  return true;
}
...
// --- Board columns (project-level, superadmin-owned) ---
// One layout per project, shared by every user and by every version-scoped board.
// Null means "never set" — the client falls back to DEFAULT_COLUMNS.

async function getBoardColumns(slug) {
  const cols = await getKv().get(`columns:${slug}`);
...
function normalizeColumns(columns) {
  ...
  for (const c of columns) {
    ...
    out.push({
      status,
      label: (typeof c.label === 'string' && c.label.trim()) || status,
...
async function setBoardColumns(slug, columns) {
  const normalized = normalizeColumns(columns);
  if (!normalized) return null;
  await getKv().set(`columns:${slug}`, normalized);
  return normalized;
}

// --- Task categories (project-level, superadmin-owned) ---
// The middle axis of the board: story lane › category rail › status column.
// A category is a *field* on a task (`task.category` holds an id), never a task
// itself — so "Backend" is one thing per project rather than one throwaway parent
// task under every story.
//
// `id` is an immutable slug, `name` is the renameable display text — the same
// split as a column's `status`/`label`, so a rename never touches a task.
// Null means "never configured"; an empty array means "configured, then emptied",
// and both are legitimate. The client renders no rails in either case.

async function getTaskCategories(slug) {
  const cats = await getKv().get(`categories:${slug}`);
...
function normalizeCategories(categories) {
  ...
  for (const c of categories) {
    ...
    out.push({
      id,
      name: (typeof c.name === 'string' && c.name.trim()) || id,
      color: typeof c.color === 'string' ? c.color : '#94a3b8',
    });
  }
  return out;
...
// Unlike setBoardColumns, an empty list is a valid save — deleting the last
// category is allowed, and leaves the project with no rails.
async function setTaskCategories(slug, categories) {
  const normalized = normalizeCategories(categories);
  if (!normalized) return null;
  await getKv().set(`categories:${slug}`, normalized);
  return normalized;
}
...
module.exports = {
  ...
  listVersions, getVersion, saveVersion, createVersion, deleteVersion,
  ...
  getBoardColumns, setBoardColumns,
