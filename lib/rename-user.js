...
async function cascadeRename(oldName, newName) {
  ...
  // Group membership is stored by name, so it has to follow the rename too.
  try {
    const { getGroupsForUser, updateGroup } = require('./group-store')
    const groups = await getGroupsForUser(oldName)
    for (const g of groups) {
      await updateGroup(g.id, { members: g.members.map(m => (m === oldName ? newName : m)) })
    }
    await kv.del(`user-groups:${oldName}`)
  } catch {}
