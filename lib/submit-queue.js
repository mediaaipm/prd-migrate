...
function writeQueue(items) {
  ...
  try {
    ...
    // Quota exceeded. Dropping the queue would lose the user's work, so keep the
    // in-memory copy authoritative and let the drain flush it; only durability
...
// Pending mutations only — what the unload guard and the optimistic overlay care
// about. Parked items are failures the user has been told about; they must not keep
// the "saving your progress" warning up forever.
export function getPending() {
  return readQueue().filter(i => i.status === 'pending')
}
...
function emitSync(item, response) {
  syncListeners.forEach(fn => { try { fn(item, response) } catch {} })
}
