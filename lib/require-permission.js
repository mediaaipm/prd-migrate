...
// Perms open to every authenticated user (viewers included). Task create/edit is
// self-service; deletion stays superadmin-only (see require-superadmin.js).
const SELF_SERVICE_PERMS = new Set(['task:create', 'task:update'])
...
function hasPermission(req, perm) {
  ...
  if (user.name && SELF_SERVICE_PERMS.has(perm)) return true
  return false
}
...
module.exports = { requirePermission, requireProjectAccess, hasPermission, isAssignee, assigneeStatusAllowed }
